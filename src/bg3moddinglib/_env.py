...
WINDIFF_URL = ('https://www.grigsoft.com/', 'windiff.zip')
...
class bg3_modding_env:
    ...
    def __init__(self, env_root_path: str) -> None:
        ...
        self.__windiff_path = os.path.join(self.__env_root_path, "windiff")
        self.__windiff_exe = os.path.join(self.__windiff_path, "windiff.exe")
    ...
    @property
    def windiff_path(self) -> str:
        return self.__windiff_path

    @property
    def windiff_exe(self) -> str:
        return self.__windiff_exe
    ...
    def __windiff_exists(self) -> bool:
        return os.path.isdir(self.__windiff_path) and os.path.isfile(self.__windiff_exe)

    def __get_windiff(self) -> None:
        try:
            if self.__windiff_exists():
                return True
            temp_path = os.path.join(os.getenv("TEMP"), "bg3modding" + str(time.time()))
            url = WINDIFF_URL[0] + WINDIFF_URL[1]
            dest_file_path = os.path.join(temp_path, WINDIFF_URL[1])
            try:
                bg3_modding_env.download_file(url, dest_file_path)
                with zipfile.ZipFile(dest_file_path) as zip:
                    zip.extractall(path=self.__windiff_path)
            finally:
                shutil.rmtree(temp_path)
        except Exception as exc:
            raise RuntimeError(f"Failed to download and extract windiff") from exc
