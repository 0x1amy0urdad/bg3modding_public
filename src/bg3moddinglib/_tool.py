...
class bg3_modding_tool:
    ...
    def convert_lsf_to_lsx(self, target: str) -> str:
        if not target.endswith(".lsf"):
            raise ValueError(f"Unexpected input file: {target}; expected an .lsf file.")
    ...
    def convert_lsx_to_lsf(self, target: str) -> str:
        if not target.endswith(".lsf.lsx"):
            raise ValueError(f"Unexpected input file: {target}; expected an .lsf.lsx file.")
        dest_path = target[:-4]
        if os.path.isfile(dest_path):
            os.unlink(dest_path)
        proc = subprocess.run(
